...
if(isset($_POST['submit'])){
	foreach ($_POST['sort'] as $key => $val) {
		if(!is_numeric($val) || !is_numeric($key)){
			skip('father_module.php','error','排序参数错误!');exit();
		}
		$query[]="update sfk_father_module set sort={$val} where id={$key}";
	}
	if(execute_multi($link,$query,$error)){
		skip('father_module.php','ok','排序修改成功!'); exit();
	}else{
		skip('father_module.php','error','$error');exit();
	}
}
...
$html=<<<A
                	<tr>
				    	...
				    	<td><a href=" ">[访问]</a>&nbsp;&nbsp;<a href="father_module_update.php?id={$data['id']}">[修改]</a>&nbsp;&nbsp;<a href="$delete_url">[删除]</a></td>
                 	</tr>
A;
					echo $html;		    
				}

...
$page：分页的get参数
*/
function page($count,$page_size,$num_btn=10,$page='page'){ //分页函数
	if(!isset($_GET[$page]) || !is_numeric($_GET[$page]) || $_GET[$page]<1){
		$_GET[$page]=1;
	}
	if($count==0){ //如果没有记录，则直接返回空的字符串
		$data=array(
			'limit'=>'',
			'html'=>''
	);
	...
	//总页数
	$page_num_all=ceil($count/$page_size);//总页数=向上取整（总条数/每页条数）
	...
	if($num_btn>=$page_num_all){
		...
		for($i=1;$i<=$page_num_all;$i++){//这边的$page_num_all是限制循环次数以控制显示按钮数目的变量,$i是记录页码号
			if($_GET[$page]==$i){ //如果是当前页，则不能点击超链接
				$html[$i]="<span>{$i}</span>";
			}else{
		...
		if(count($html)>=3){//如果按钮数目大于等于3的时候做省略号效果
			reset($html);
			$key_first=key($html);
			end($html);
			$key_end=key($html);
	...
	$html=implode(' ',$html); //把数组中的元素用空格连起来
	$data=array(
			'limit'=>$limit,
			'html'=>$html
	);

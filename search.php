...
$_GET['keyword']=trim($_GET['keyword']);//去掉搜索框中的左右空格
$_GET['keyword']=escape($link,$_GET['keyword']);
$query="select count(*) from sfk_content where title like '%{$_GET['keyword']}%'";
$count_title=num($link,$query);
$query="select count(*) from sfk_content where content like '%{$_GET['keyword']}%'";
$count_content=num($link,$query);

$count_all=$count_title+$count_content;
...
<div id="main" class="auto">
	<div id="left">
		...
		<ul class="postsList">
			<?php 
			$query="select
			sfk_content.title,sfk_content.id,sfk_content.content,sfk_content.time,sfk_content.times,sfk_content.member_id,sfk_member.name,sfk_member.photo
			from sfk_content,sfk_member where
			sfk_content.title like '%{$_GET['keyword']}%' and
			sfk_content.content like '%{$_GET['keyword']}%' and
				...
				<div class="subject">
					...
					<p>
						...
						if(check_user($member_id,$data_content['member_id'],$is_manage_login)){
							$return_url=urlencode($_SERVER['REQUEST_URI']);//获取当前页面
